...
def main():
    ...
    def compute_metrics(sources, preds, labels):
        ...
        decoded_preds, decoded_labels, decoded_src = postprocess_text(decoded_preds, decoded_labels, decoded_src)
        result = metric.compute(sources=decoded_src, predictions=decoded_preds, references=decoded_labels)
    ...
    if training_args.do_predict:
        ...
        for _ in tqdm(range(pred_steps), desc="Predicting...", position=2, leave=False):
            ...
            if data_args.predict_with_generate:
                generated_ids = p_generate_step(state.params, batch)
                pred_sources.extend(jax.device_get(batch['input_ids'].reshape(-1, batch['input_ids'].shape[-1])))
        ...
        rouge_desc = ""
        if data_args.predict_with_generate:
            rouge_metrics = compute_metrics(pred_sources, pred_generations, pred_labels)
